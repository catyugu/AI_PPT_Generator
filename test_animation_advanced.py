...
# --- 测试主程序 (已更新) ---
def run_full_suite_test():
    """测试包含进入和退出效果的完整动画系统。"""
    logging.info("--- 开始完整动画套件功能测试 ---")
    prs = Presentation()
    slide = prs.slides.add_slide(prs.slide_layouts[5])
    slide.shapes.title.text = "进入与退出动画效果"

    # 添加第一个形状：淡入，然后淡出
    shape1 = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(1), Inches(2), Inches(3), Inches(1.5))
    shape1.text = "淡入 & 淡出"
    add_animation(shape1, "fadeIn", duration_ms=750)
    add_animation(shape1, "fadeOut", duration_ms=750)

    # 添加第二个形状：飞入，然后飞出
    shape2 = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(6), Inches(4), Inches(3), Inches(1.5))
    shape2.text = "飞入 & 飞出"
    add_animation(shape2, "flyIn", direction="fromLeft", duration_ms=1000)
    add_animation(shape2, "flyOut", direction="toRight", duration_ms=1000)

    output_file = "test_animation_full_suite.pptx"
